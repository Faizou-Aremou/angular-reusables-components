...
export function embelishSlipInTwo<T>(sequence: Array<T>): {
  ...
  switch (sequence.length) {
    ...
    case 1:
      return { sequencePart1: [], sequencePart2: [sequence[0]], halfOfSequenceSize: 0, sequenceSize: 1 };
    case 2:
      return { sequencePart1: [sequence[0]], sequencePart2: [sequence[1]], halfOfSequenceSize: 1, sequenceSize: 2 };
    default:
      const { sequencePart1, sequencePart2, halfOfSequenceSize, sequenceSize } = embelishSlipInTwo(tail(sequence));
...
export function InterClassement<T>(
  ...
  return (head(sequence1) as T) <= (head(sequence2) as T)
    ? prepend(head(sequence1) as T, InterClassement(isInf, tail(sequence1), sequence2))
    : prepend(head(sequence2) as T, InterClassement(isInf, sequence1, tail(sequence2)));
}
/**
